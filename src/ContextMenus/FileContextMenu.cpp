...
std::vector<std::shared_ptr<UIElement>> FileContextMenu::make()
{
    ...
    UIButton DeleteTagFromFileButton(this->comm, L"Delete tag from file", 12, RGBA(100, 100, 100), 5);
    DeleteTagFromFileButton.getElement()->anchors[Direction::Right] = true;
    auto DeleteTagFromFile = [filePath = this->_filePath](std::shared_ptr<UIElement> &el, const SDL_Event &e)
    {
        el->comm->state->tagActionMode = (int)TagActionMode::DeleteFromFile;
        el->comm->state->addTagPath = filePath;
        el->comm->appEventsQueue.push_back(std::make_shared<AppEvent>(AppEventType::CloseContextMenu));
        el->comm->appEventsQueue.push_back(std::make_shared<AppEvent>(AppEventType::TagActionChange));
        return EventResult<std::shared_ptr<UIElement>>();
    };
    DeleteTagFromFileButton.getElement()->events.addHandler((int)CustomEvent::MOUSE_CLICK, DeleteTagFromFile);

    return std::vector<std::shared_ptr<UIElement>>(
        {
            ShowInExplorerButton.getElement(),
            AddTagToFileButton.getElement(),
            DeleteTagFromFileButton.getElement(),
        });
}