...
{
    Setter::setInRange(x, 0, this->surface->w - 1);
    Setter::setInRange(y, 0, this->surface->h - 1);
    uint8_t *pixels = (uint8_t *)this->surface->pixels;
    pixels[(x * this->surface->format->BytesPerPixel) + y * this->surface->pitch + 0] = this->color.r;
    pixels[(x * this->surface->format->BytesPerPixel) + y * this->surface->pitch + 1] = this->color.g;
    pixels[(x * this->surface->format->BytesPerPixel) + y * this->surface->pitch + 2] = this->color.b;
    pixels[(x * this->surface->format->BytesPerPixel) + y * this->surface->pitch + 3] = this->color.a;
}

void SurfaceEditor::setSurfacePixelRow(int x1, int x2, int y)
{
    Setter::swap(x1, x2, x1 > x2);
    Setter::setInRange(x1, 0, this->surface->w - 1);
    Setter::setInRange(x2, 0, this->surface->w - 1);
    Setter::setInRange(y, 0, this->surface->h - 1);
    uint8_t *pixels = (uint8_t *)this->surface->pixels;
    for (int x = x1; x <= x2; x++)
    {
...
void SurfaceEditor::setSurfacePixelColumn(int x, int y1, int y2)
{
    ...
    Setter::setInRange(x, 0, this->surface->w - 1);
    Setter::setInRange(y1, 0, this->surface->h - 1);
    Setter::setInRange(y2, 0, this->surface->h - 1);
    uint8_t *pixels = (uint8_t *)this->surface->pixels;
    for (int y = y1; y <= y2; y++)
    {
        pixels[(x * this->surface->format->BytesPerPixel) + y * this->surface->pitch + 0] = this->color.r;
...
void SurfaceEditor::drawWithCoordsVector(CoordsVector coords, int x1, int y1, int x2, int y2)
{
    ...
    Setter::setIf(y2, this->surface->h, y2 < 0);
